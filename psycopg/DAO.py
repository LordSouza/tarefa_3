...
def search_customers(name: str):
    ...
    try:
        db = next(get_db())
        return db.execute(
            "SELECT * FROM northwind.customers WHERE contactname LIKE %s", (name,)
        ).fetchall()
    except Exception as e:
        print(e)
...
def search_products(productname: str):
    ...
    try:
        db = next(get_db())
        return db.execute(
            "SELECT * FROM northwind.products WHERE productname LIKE %s", (productname,)
        ).fetchall()
    except Exception as e:
        print(e)
...
def search_employee(name: str):
    ...
    try:
        db = next(get_db())
        return db.execute(
            "SELECT * FROM northwind.employees WHERE lastname LIKE %s OR firstname LIKE %s",
            (name, name),
        ).fetchall()
    except Exception as e:
        print(e)
